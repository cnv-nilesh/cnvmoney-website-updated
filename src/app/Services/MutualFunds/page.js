...
const page = () => {
  ...
  return (
    <div className="container p-2 mt-5 mx-auto">
      <h1 className="w-50 p-2 text-center bg-slate-200">Mutual Funds Links</h1>
      <div className="w-full h-1 bg-slate-100 mt-5 mb-4"></div>
      <div className="bg-white drop-shadow-md p-4 mt-4 flex flex-wrap  justify-center gap-4">
        {images.map((item, id) => (
          <div
            className="mt-4 mb-4 drop-shadow-md cursor-pointer flex justify-center items-center"
            key={id}
          >
            <a href={item.links}>
              <Image
                ...
                height={20}
                alt="amc Images"
                className="bg-amber-50 w-45 h-20 object-cover"
              ></Image>
            </a>
          </div>
        ))}
