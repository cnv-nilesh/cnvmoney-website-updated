...
const Lumpsumcalculator = () => {
  const [monthlyInvestment, setMonthlyInvestment] = useState(5000000);
  ...
  const calculateSip = () => {
    ...
    const returns = futureValue - monthlyInvestment;

    setInvestedAmount(monthlyInvestment);
  ...
  const handleDownloadPDF = () => {
    ...
    Promise.all([
      ...
      .then(([canvas, logoBase64]) => {
        ...
        doc.setCharSpace(0);
        const body = [];
        const r = expectedReturnRate / 100 / 12; // monthly interest rate
        ...
        for (let year = 1; year <= timePeriod; year++) {
          ...
          for (let i = 1; i <= months; i++) {
            marketValue += monthlyInvestment * Math.pow(1 + r, months - i + 1);
          }
          ...
          body.push([
            ...
            `Rs ${Math.round(gain).toLocaleString("en-IN")}`,
            `Rs ${Math.round(marketValue).toLocaleString("en-IN")}`,
          ]);
        }
    ...
    <div className="bg-white p-6 drop-shadow-lg rounded flex flex-wrap gap-5 items-center">
      ...
      >
        ...
        <div className="space-y-1 text-center mt-8 lg:mt-28">
          ...
          <div className="flex text-green-700 text-[16px] space-x-1 font-semibold">
            <span>Returns : </span>
            <span>
              ₹{" "}
              {new Intl.NumberFormat("en-IN").format(
          ...
          <div className="flex text-[#0143a2] text-[17px] font-semibold">
